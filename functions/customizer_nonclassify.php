 ...
 * This function generates the layout when page is reloaded
 *
 * @since Twenty Fifteen 1.0
 *
...
function charmeem_color_scheme_css() {
	...
	$settings = get_setting();
		foreach ($settings['color_template'] as $templ) {
			$color = get_theme_mod($templ, 'default');
			$colors[$templ] = $color;
			}
			//var_dump($colors);
			
	//$bb = get_theme_mod( 'box_background', 'default' );
	//$men = get_theme_mod( 'menu', 'default' );
	//$po = get_theme_mod( 'post', 'default' );
...
//var_dump($colors);
	...
	// Without this line the change in the plugin is override by the current theme style
